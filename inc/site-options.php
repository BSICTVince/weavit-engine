...
/**
 * Site Options is a tabbed shell — "General" (this file) is always present;
 * a theme registers additional tabs (e.g. Navigation) via this filter
 * instead of this generic, reusable-across-themes file growing theme-
 * specific sections. Matches the tab pattern already used by the SEO tools
 * screen (Redirects / .htaccess).
 */
function bootg_site_options_tabs() {
	$tabs = array(
		'general' => array( 'label' => 'General', 'render' => 'bootg_render_general_options_tab' ),
	);
	return apply_filters( 'bootg_site_options_tabs', $tabs );
}
	...
	<div class="wrap">
		...
		<h2 class="nav-tab-wrapper">
			<?php foreach ( $tabs as $key => $tab ) : ?>
				<a href="<?php echo esc_url( bootg_site_options_tab_url( $key ) ); ?>" class="nav-tab <?php echo $current === $key ? 'nav-tab-active' : ''; ?>"><?php echo esc_html( $tab['label'] ); ?></a>
			<?php endforeach; ?>
		</h2>
...
function bootg_render_general_options_tab() {
	...
	<form action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" method="post" style="margin-bottom:2em;">
		...
		<p class="description">Or use <a href="<?php echo esc_url( admin_url( 'customize.php?autofocus[section]=title_tagline' ) ); ?>">Appearance &rarr; Customize &rarr; Site Identity</a> to upload a logo file directly. Logo <strong>size</strong> is under the Navigation tab.</p>
	</form>
